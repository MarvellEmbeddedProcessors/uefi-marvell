...
STATIC MV_SOC_FUSE_DESC MvApFuseDesc[MV_AP_FUSE_COUNT] = {
  {0, HighDensity, BankRW, MV_AP_FUSE_CTRL_BASE, MV_AP_FUSE_HD_MEN_BASE, MV_FUSE_HD_ROW_CNT, MV_FUSE_HD_ROW_LEN, 16, L"AP0-HD0", 0},
  {1, LowDensity, BankRO, MV_AP_FUSE_CTRL_BASE, MV_AP_FUSE_LD_MEN_BASE, MV_FUSE_LD_ROW_CNT, MV_FUSE_LD_ROW_LEN, 0, L"AP0-LD0", 0},
  {2, LowDensity, BankRW, MV_AP_FUSE_CTRL_BASE, MV_AP_FUSE_LD_MEN_BASE, MV_FUSE_LD_ROW_CNT, MV_FUSE_LD_ROW_LEN, 0, L"AP0-LD1", 0}
};
  ...
  )
{
  ...
  UINT8 CpCount = FixedPcdGet8 (PcdMaxCpCount);
  UINT8 CpIndex, FuseIndex = 0, LdIndex = 0;

  Desc = AllocateZeroPool ((CpCount * MV_FUSE_COUNT_PER_CP + MV_AP_FUSE_COUNT) * sizeof (MV_SOC_FUSE_DESC));
  if (Desc == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: Cannot allocate memory\n", __FUNCTION__));
    return EFI_OUT_OF_RESOURCES;
  }
